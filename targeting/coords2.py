...
def moveToCoords():
    while (True):
        ...
        ################# Only get here if we have enough info to move the laser ########################
        ...
        xSteps = xDiff / ratio
        ySteps = yDiff / ratio
        stepTol = tolerance / ratio

        #max iterations used in the for loop
        maxSteps = max(abs(xDiff),abs(yDiff)) / ratio

        #convert the pixel coords into steps, then use a for loop to step the appropriate amount
        laserX = laserXC / ratio
        laserY = laserYC / ratio
        linkX = linkXC / ratio
        linkY = linkYC / ratio

        #take maxSteps number of steps
        #LATER: try the y = mx thing
        prev_link = link_coords
        prev_laser = laser_coords

        for i in range(int(maxSteps)):

            #if the coords of link have changed, start the while loop all over again
            if(link_coords[0] != linkXC and link_coords[1] != linkYC):
                #Not sure if break is the correct one to have here
                break
                #continue?
