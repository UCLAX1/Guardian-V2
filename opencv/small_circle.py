...
while (True):
    ...
    frame_index_str = "000" + str(frame_index)
    if len(frame_index_str) > 4:
        frame_index_str = frame_index_str[1:]
    frame_str = "/Users/audi/Downloads/Laser_Set_1/set3/image_" + frame_index_str + ".jpg"
    frame_index_str = str(frame_index)
    #frame_str = "/Users/audi/Downloads/Link_Laser2/pi_" + frame_index_str + ".jpg"
    frame = cv2.imread(frame_str)
    if frame is None:
        print("Failed to read image {}".format(frame_str))
        frame_index += 1
        continue
